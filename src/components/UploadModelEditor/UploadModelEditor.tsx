...
export const UploadModelEditor = () => {
  ...
  const handleDeleteStep = (stepId: string) => {
    if (window.confirm(language === 'pl' ? 'Czy na pewno chcesz usunąć ten krok?' : 'Are you sure you want to delete this step?')) {
      deleteStep(stepId);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{isPickMode ? (language === 'pl' ? 'Tryb wyboru — kliknij element modelu' : 'Pick mode') : (language === 'pl' ? 'Obracaj kamerą swobodnie' : 'Rotate camera freely')}</p>
              </div>
            </div>
            {editingStep && !isPickMode && (
              <button
                ...
                title={language === 'pl' ? 'Zapisz obecną pozycję kamery dla tego kroku' : 'Save camera position for this step'}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                {editingStep.cameraCaptured ? (language === 'pl' ? '✓ Kamera zapisana' : '✓ Camera saved') : (language === 'pl' ? 'Zapisz kamerę' : 'Save camera')}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="px-5 py-4 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-slate-800 text-base">{language === 'pl' ? 'Kroki dokumentacji' : 'Documentation steps'}</h2>
                <p className="text-xs text-slate-500">{project.steps.length} {language === 'pl' ? 'kroków' : 'steps'}</p>
              </div>
            </div>
            <button
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {!editingStep && (
            <button onClick={openAddForm} className="w-full p-3 border-2 border-dashed border-blue-300 rounded-lg hover:border-blue-500 hover:bg-blue-50 transition flex items-center gap-2 text-blue-500 hover:text-blue-700">
              ...
              <span className="text-sm font-medium">{language === 'pl' ? 'Dodaj nowy krok' : 'Add new step'}</span>
            </button>
          )}
          ...
          {editingStep && (
            <div className="border border-blue-300 rounded-lg p-4 bg-blue-50 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-slate-800 text-sm">{isAddingStep ? (language === 'pl' ? 'Nowy krok' : 'New step') : (language === 'pl' ? 'Edytuj krok' : 'Edit step')}</h3>
                {isPickMode ? (
                  <span className="text-xs text-blue-600 font-medium animate-pulse">{language === "pl" ? "← Kliknij element…" : "← Click element…"}</span>
                ) : (
                  <button onClick={handleEnterPickMode} className="text-xs text-blue-600 hover:text-blue-800 font-medium underline" title={language === "pl" ? "Kliknij ponownie element modelu 3D" : "Click model element again"}>
              ...
              {!editingStep.focusMeshName && !isPickMode && (
                <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  ...
                  <span className="text-xs text-amber-700">{language === 'pl' ? 'Brak wybranego elementu — krok będzie ogólny' : 'No element selected — step will be general'}</span>
                </div>
              )}

              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-medium text-slate-600">{language === 'pl' ? 'Tytuł' : 'Title'}</label>
                ...
                <input
                  ...
                  placeholder={language === 'pl' ? 'Tytuł kroku…' : 'Step title…'}
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500 bg-white border border-slate-200 rounded-lg px-3 py-2">
                ...
                {editingStep.cameraCaptured
                  ? <span className="text-green-600 font-medium">{language === 'pl' ? '✓ Pozycja kamery zapisana' : '✓ Camera position saved'}</span>
                  : <span>Ustaw kamerę i kliknij „Zapisz kamerę" powyżej</span>
                }
              </div>

              <div className="flex gap-2">
                <button onClick={handleSaveStep} disabled={!editingStep.title.trim()} className="flex-1 px-3 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition disabled:bg-gray-300 disabled:cursor-not-allowed">{language === 'pl' ? 'Zapisz krok' : 'Save step'}</button>
                <button onClick={handleCancelEdit} className="flex-1 px-3 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300 transition">{language === 'pl' ? 'Anuluj' : 'Cancel'}</button>
              </div>
            </div>
          )}
          ...
          {project.steps.length === 0 && !editingStep && (
            <div className="text-center py-8 text-slate-400">
              ...
              <p className="text-sm">{language === 'pl' ? 'Brak kroków' : 'No steps'}</p>
          ...
          {project.steps.map((step, index) => (
            ...
            >
              ...
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={(e) => { e.stopPropagation(); openEditForm(step); }} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition" title={language === 'pl' ? 'Edytuj krok' : 'Edit step'}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button onClick={(e) => { e.stopPropagation(); handleDeleteStep(step.id); }} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition" title={language === 'pl' ? 'Usuń krok' : 'Delete step'}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
