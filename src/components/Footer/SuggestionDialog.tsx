...
export const SuggestionDialog = ({ onClose }: SuggestionDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t.suggestion.error);
    } finally {
      setIsSending(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 rounded-t-2xl px-6 py-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">{t.suggestion.title}</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          {sent ? (
            <div className="text-center py-6">
              ...
              <p className="text-gray-800 font-medium text-lg">{t.suggestion.success}</p>
              <p className="text-gray-500 text-sm mt-1">Twoja opinia pomoże nam ulepszać system.</p>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit}>
              <p className="text-gray-600 text-sm mb-4">
                Masz pomysł na ulepszenie systemu? Opisz co można zmienić lub poprawić.
              </p>
              <textarea
                value={content}
              ...
              <div className="mt-4 flex justify-end gap-3 border-t border-gray-100 pt-4">
                ...
                >
                  {isSending ? t.common.loading : t.suggestion.submit}
                </button>
              </div>
            </form>
          )}
