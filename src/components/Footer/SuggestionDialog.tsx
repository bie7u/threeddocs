...
export const SuggestionDialog = ({ onClose }: SuggestionDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : lt.sendError);
    } finally {
      setIsSending(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 rounded-t-2xl px-6 py-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">{lt.title}</h2>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white transition-colors"
            aria-label={lt.closeAriaLabel}
          >
        ...
        <div className="p-6">
          {sent ? (
            <div className="text-center py-6">
              ...
              <p className="text-gray-800 font-medium text-lg">{lt.successTitle}</p>
              <p className="text-gray-500 text-sm mt-1">{lt.successDescription}</p>
              <button
                onClick={onClose}
