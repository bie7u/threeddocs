...
export const StepProperties = () => {
  ...
  const handleDelete = () => {
    if (selectedStepId && window.confirm(t('stepProperties.confirmDelete'))) {
  ...
  const handleAddNewStep = () => {
    const newStep: InstructionStep = {
      ...
      title: t('stepProperties.newStep'),
  ...
  const getShapeButtonLabel = (): string => {
    ...
    if (formData.shapeType === 'custom3dElement') {
      ...
      return `🧩 ${el?.name ?? t('stepProperties.custom3dElementFallback')}`;
    }
    if (formData.shapeType === 'uploadedModel') {
      const m = uploadedModels.find((u) => u.id === formData.uploadedModelId);
      return `📤 ${m?.name ?? t('stepProperties.uploadedModelFallback')}`;
    }
    return labels[formData.shapeType ?? 'cube'] ?? `📦 ${t('shapeLabels.cube')}`;
  };
