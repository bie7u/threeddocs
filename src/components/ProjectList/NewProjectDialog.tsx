...
export const NewProjectDialog = ({ onClose, onCreateProject, isCreating = false, createError = '' }: NewProjectDialogProps) => {
  ...
  const handleTypeSelect = (type: ProjectType) => {
    ...
    setStep('configure');
    setProjectName(type === 'builder' ? t('newProject.defaultNameBuilder') : t('newProject.defaultNameUpload'));
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      alert(t('newProject.fileTooLarge'));
      e.target.value = '';
      return;
    }

    if (!file.name.match(/\.(gltf|glb)$/i)) {
      alert(t('newProject.invalidFile'));
      e.target.value = '';
      return;
    }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{t('newProject.subtitle')}</p>
              </div>
            </div>
        ...
        <div className="p-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2 mt-2">
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{t('newProject.builderTag1')}</span>
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{t('newProject.builderTag2')}</span>
                    </div>
                  </div>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2 mt-2">
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">{t('newProject.uploadTag1')}</span>
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">{t('newProject.uploadTag2')}</span>
                    </div>
                  </div>
          ...
          {step === 'configure' && selectedType && (
            <div className="space-y-4">
              ...
              <div className={`flex items-center gap-3 p-3 rounded-xl ${selectedType === 'builder' ? 'bg-blue-50 border border-blue-200' : 'bg-green-50 border border-green-200'}`}>
                ...
                <div>
                  <p className={`text-sm font-semibold ${selectedType === 'builder' ? 'text-blue-700' : 'text-green-700'}`}>
                    {selectedType === 'builder' ? t('newProject.builderSelected') : t('newProject.uploadSelected')}
                  </p>
                  <p className="text-xs text-slate-500">
                    {selectedType === 'builder' ? t('newProject.builderSelectedDesc') : t('newProject.uploadSelectedDesc')}
                  </p>
                </div>
              </div>
              ...
              {selectedType === 'upload' && (
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('newProject.modelFile')}</label>
                  <div
                  ...
                  >
                    ...
                    {isUploading ? (
                      <div className="text-center">
                        ...
                        <p className="text-sm text-slate-500">{t('newProject.reading')}</p>
                      </div>
                    ) : uploadedModelUrl ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{t('newProject.clickToChange')}</p>
                        </div>
                      </div>
                    ) : (
                      <div className="text-center">
                        ...
                        <p className="text-sm font-medium text-slate-600">{t('newProject.clickToSelect')}</p>
                        <p className="text-xs text-slate-400 mt-0.5">{t('newProject.fileHint')}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isCreating ? t('newProject.creating') : t('newProject.createProject')}
                </button>
              </div>
            </div>
          )}
