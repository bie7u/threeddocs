...
export const NewProjectDialog = ({ onClose, onCreateProject, isCreating = false, createError = '' }: NewProjectDialogProps) => {
  ...
  const handleTypeSelect = (type: ProjectType) => {
    ...
    setStep('configure');
    setProjectName(type === 'builder' ? t('newProjectDialog.defaultBuilderName') : t('newProjectDialog.defaultUploadName'));
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      alert(t('newProjectDialog.fileTooLarge'));
      e.target.value = '';
      return;
    }

    if (!file.name.match(/\.(gltf|glb)$/i)) {
      alert(t('newProjectDialog.fileInvalidType'));
      e.target.value = '';
      return;
    }
    ...
    reader.onerror = () => {
      alert(t('newProjectDialog.fileReadError'));
      setIsUploading(false);
    };
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">{t('newProjectDialog.title')}</h2>
                <p className="text-xs text-slate-400">{t('newProjectDialog.subtitle')}</p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium transition-all ${step === 'configure' ? 'bg-blue-500 text-white' : 'bg-slate-700 text-slate-300'}`}>
              ...
              {t('newProjectDialog.stepConfigure')}
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          {step === 'choose-type' && (
            <div className="space-y-4">
              <p className="text-sm text-slate-600 mb-4">{t('newProjectDialog.chooseTypeLabel')}</p>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-800 text-base mb-1">{t('newProjectDialog.builderTitle')}</h3>
                    <p className="text-sm text-slate-500">{t('newProjectDialog.builderDesc')}</p>
                    <div className="flex gap-2 mt-2">
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{t('newProjectDialog.builderTag1')}</span>
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{t('newProjectDialog.builderTag2')}</span>
                    </div>
                  </div>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-800 text-base mb-1">{t('newProjectDialog.uploadTitle')}</h3>
                    <p className="text-sm text-slate-500">{t('newProjectDialog.uploadDesc')}</p>
                    <div className="flex gap-2 mt-2">
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">{t('newProjectDialog.uploadTag1')}</span>
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">{t('newProjectDialog.uploadTag2')}</span>
                    </div>
                  </div>
          ...
          {step === 'configure' && selectedType && (
            <div className="space-y-4">
              ...
              <div className={`flex items-center gap-3 p-3 rounded-xl ${selectedType === 'builder' ? 'bg-blue-50 border border-blue-200' : 'bg-green-50 border border-green-200'}`}>
                ...
                <div>
                  <p className={`text-sm font-semibold ${selectedType === 'builder' ? 'text-blue-700' : 'text-green-700'}`}>
                    {selectedType === 'builder' ? t('newProjectDialog.builderSelected') : t('newProjectDialog.uploadSelected')}
                  </p>
                  <p className="text-xs text-slate-500">
                    {selectedType === 'builder' ? t('newProjectDialog.builderSelectedSubtitle') : t('newProjectDialog.uploadSelectedSubtitle')}
                  </p>
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('newProjectDialog.projectNameLabel')}</label>
                <input
                  type="text"
                  value={projectName}
              ...
              {selectedType === 'upload' && (
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('newProjectDialog.modelFileLabel')}</label>
                  <div
                  ...
                  >
                    ...
                    {isUploading ? (
                      <div className="text-center">
                        ...
                        <p className="text-sm text-slate-500">{t('newProjectDialog.fileUploading')}</p>
                      </div>
                    ) : uploadedModelUrl ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{t('newProjectDialog.fileChangeHint')}</p>
                        </div>
                      </div>
                    ) : (
                      <div className="text-center">
                        ...
                        <p className="text-sm font-medium text-slate-600">{t('newProjectDialog.fileSelectHint')}</p>
                        <p className="text-xs text-slate-400 mt-0.5">{t('newProjectDialog.fileHint')}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isCreating ? t('newProjectDialog.createLoading') : t('newProjectDialog.createBtn')}
                </button>
              </div>
            </div>
          )}
