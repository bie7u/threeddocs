...
export const NewProjectDialog = ({ onClose, onCreateProject, isCreating = false, createError = '' }: NewProjectDialogProps) => {
  ...
  const { t } = useLanguage();
  const lt = t.newProject;

  const handleTypeSelect = (type: ProjectType) => {
    setSelectedType(type);
    setStep('configure');
    setProjectName(type === 'builder' ? lt.defaultBuilderName : lt.defaultUploadName);
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      alert(lt.fileTooLarge);
      e.target.value = '';
      return;
    }

    if (!file.name.match(/\.(gltf|glb)$/i)) {
      alert(lt.invalidFileType);
      e.target.value = '';
      return;
    }
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">{lt.title}</h2>
                <p className="text-xs text-slate-400">{lt.subtitle}</p>
              </div>
            </div>
        ...
        <div className="p-6">
          {step === 'choose-type' && (
            <div className="space-y-4">
              <p className="text-sm text-slate-600 mb-4">{lt.howToCreate}</p>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-800 text-base mb-1">{lt.build3DTitle}</h3>
                    <p className="text-sm text-slate-500">{lt.build3DDesc}</p>
                    <div className="flex gap-2 mt-2">
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{lt.build3DTag1}</span>
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{lt.build3DTag2}</span>
                    </div>
                  </div>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-800 text-base mb-1">{lt.uploadTitle}</h3>
                    <p className="text-sm text-slate-500">{lt.uploadDesc}</p>
                    <div className="flex gap-2 mt-2">
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">{lt.uploadTag1}</span>
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">{lt.uploadTag2}</span>
                    </div>
                  </div>
          ...
          {step === 'configure' && selectedType && (
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">{lt.projectNameLabel}</label>
                <input
                  type="text"
                  value={projectName}
              ...
              {selectedType === 'upload' && (
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">{lt.modelFileLabel}</label>
                  <div
                  ...
                  >
                    ...
                    {isUploading ? (
                      <div className="text-center">
                        ...
                        <p className="text-sm text-slate-500">{lt.loadingFile}</p>
                      </div>
                    ) : uploadedModelUrl ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{lt.clickToChangeFile}</p>
                        </div>
                      </div>
                    ) : (
                      <div className="text-center">
                        ...
                        <p className="text-sm font-medium text-slate-600">{lt.clickToSelectFile}</p>
                        <p className="text-xs text-slate-400 mt-0.5">{lt.fileRestrictions}</p>
                      </div>
                    )}
                  </div>
                </div>
