        ...
        <div className="h-16 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white flex items-center justify-between px-6 shadow-xl border-b border-slate-700">
          <div className="flex items-center gap-4">
            {onBackToProjectList && (
              <button onClick={onBackToProjectList} className="px-4 py-2 bg-slate-700/50 backdrop-blur-sm rounded-lg hover:bg-slate-600/50 transition-all duration-200 flex items-center gap-2 border border-slate-600/30 shadow-lg" title={t('mainLayout.backToProjectsTitle')}>
                ...
                <span className="text-sm font-medium">{isGuestMode ? t('mainLayout.backButtonGuest') : t('mainLayout.backButton')}</span>
              </button>
            )}
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold">{t('mainLayout.doc3DTitle')}{isGuestMode && <span className="ml-2 text-xs font-normal text-yellow-300 bg-yellow-500/20 px-2 py-0.5 rounded-full">{t('mainLayout.guestModeBadge')}</span>}</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="px-4 py-2 bg-green-500/20 backdrop-blur-sm rounded-lg border border-green-500/30 shadow-lg flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-green-300">{t('mainLayout.editorModeBadge')}</span>
            </div>
          </div>
        </div>
      ...
      <div className="h-16 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white flex items-center justify-between px-6 shadow-xl border-b border-slate-700">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold">{t('mainLayout.builderTitle')}{isGuestMode && <span className="ml-2 text-xs font-normal text-yellow-300 bg-yellow-500/20 px-2 py-0.5 rounded-full">{t('mainLayout.guestModeBadge')}</span>}</h1>
              {project && <span className="text-xs text-slate-300 font-medium">{project.name}</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center bg-slate-700/50 rounded-lg p-1 border border-slate-600/30">
            ...
            >{t('mainLayout.modelCreator')}</button>
            <button
            ...
            >{t('mainLayout.guideCreator')}</button>
          </div>
      ...
      {isGuestMode && (
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-2 flex items-center justify-between gap-4 flex-shrink-0">
          <div className="flex items-center gap-2 text-amber-800 text-sm">
            ...
            <span className="font-medium">{t('mainLayout.guestModeBadge')}</span>
            <span className="text-amber-700">{t('mainLayout.guestBanner')}</span>
          </div>
          {onGoToLogin && (
            <button
              onClick={onGoToLogin}
            ...
            <div className="flex-shrink-0 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden flex flex-col" style={{ width: panelWidth }}>
              <div className="px-5 py-3.5 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-slate-800 text-sm">{t('mainLayout.stepPropertiesTitle')}</h2>
                    <p className="text-xs text-slate-500">{t('mainLayout.stepPropertiesSubtitle')}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="px-5 py-3.5 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 flex-shrink-0">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="font-bold text-slate-800 text-sm">{t('mainLayout.modelBuilderTitle')}</h2>
                      <p className="text-xs text-slate-500">
                        {t('mainLayout.modelBuilderSubtitle')}
                        {project && project.steps.length > 0 && (
                          <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">
                            {project.steps.length} {
                              project.steps.length === 1 ? t('mainLayout.stepCountOne') :
                              [2,3,4].includes(project.steps.length % 10) && ![12,13,14].includes(project.steps.length % 100) ? t('mainLayout.stepCountFew') :
                              t('mainLayout.stepCountMany')
                            }
                          </span>
                        )}
                      </p>
                  ...
                  <div className="text-xs text-slate-400 hidden lg:block">{t('mainLayout.dragHint')}</div>
                </div>
              </div>
            ...
            <div className="w-96 flex-shrink-0 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden flex flex-col ml-1.5">
              <div className="px-5 py-3.5 bg-gradient-to-r from-slate-900 to-slate-800 border-b border-slate-700 flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-bold text-white text-sm">{t('mainLayout.preview3DTitle')}</h2>
                    <p className="text-xs text-slate-400">{t('mainLayout.preview3DSubtitle')}</p>
                  </div>
                </div>
              </div>
                ...
                {!selectedStepId && project && project.steps.length > 0 && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="bg-black/60 backdrop-blur-sm text-white px-5 py-3 rounded-xl shadow-2xl border border-white/10 mx-4 text-center">
                      ...
                      <p className="text-sm font-medium">{t('mainLayout.noStepsHint')}</p>
                    </div>
                  </div>
                )}
                {!project || project.steps.length === 0 ? (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="text-center px-4">
                      ...
                      <p className="text-xs text-slate-600 mt-1">{t('mainLayout.addStepHint')}</p>
                    </div>
                  </div>
                ) : null}
              </div>
