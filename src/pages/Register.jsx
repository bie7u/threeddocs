...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : lt.createError);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-6 p-10 bg-white rounded-2xl shadow-2xl">
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-3 bg-white text-gray-400">{lt.orByEmail}</span>
          </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">
              {lt.nameLabel} <span className="text-gray-400 font-normal">{lt.nameOptional}</span>
            </label>
            <input
              id="name"
              name="name"
          ...
          >
            {isLoading ? lt.creatingAccount : lt.createAccount}
          </button>
        </form>
