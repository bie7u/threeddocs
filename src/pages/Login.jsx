...
const Login = () => {
  ...
  // Load Google Identity Services and render the Sign In button once the
  // session check is done and a Google Client ID is configured.
  useEffect(() => {
    ...
    const existingScript = document.querySelector(`script[src="${GIS_SRC}"]`);
    if (existingScript) {
      if (window.google?.accounts?.id) {
        initButton();
      } else {
        existingScript.addEventListener('load', initButton);
        return () => existingScript.removeEventListener('load', initButton);
      }
      return;
    }
      ...
      {checking ? (
        ...
        {GOOGLE_CLIENT_ID ? (
          ...
          <button
            type="button"
            disabled
            className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-gray-300 rounded-lg text-sm font-medium text-gray-400 bg-gray-50 cursor-not-allowed"
          >
