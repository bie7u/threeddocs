...
const AccountModal = ({ onClose }) => {
  ...
  useEffect(() => {
    getMe()
      .then(setUser)
      .catch(() => setUserError(t('settings.account.loadError')));
  }, []);
  ...
  const handleChangePassword = async (e) => {
    ...
    if (newPassword !== confirmPassword) {
      setSaveError(t('settings.account.errorPasswordMatch'));
      return;
    }
    if (newPassword.length < 8) {
      setSaveError(t('settings.account.errorPasswordLength'));
      return;
    }
    setSaving(true);
    try {
      ...
      setSaveError(err.message ?? t('settings.account.errorDefault'));
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div>
            ...
            {userError ? (
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                {user.name && (
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-medium text-gray-400 w-14 flex-shrink-0">{t('settings.account.firstName')}</span>
                    <span className="text-sm font-medium text-gray-800">{user.name}</span>
                  </div>
                )}
                <div className="flex items-center gap-3">
                  <span className="text-xs font-medium text-gray-400 w-14 flex-shrink-0">{t('settings.account.email')}</span>
          ...
          <div>
            ...
            <form onSubmit={handleChangePassword} className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">{t('settings.account.newPassword')}</label>
                <input
                  type="password"
                  value={newPassword}
                  ...
                  required
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent"
                  placeholder={t('settings.account.passwordMinChars')}
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">{t('settings.account.confirmNewPassword')}</label>
                <input
                  type="password"
              ...
              {saveSuccess && (
                <p className="text-xs text-green-600 bg-green-50 px-3 py-2 rounded-lg">{t('settings.account.successMsg')}</p>
              )}
              ...
              >
                {saving ? t('settings.account.saveLoading') : t('settings.account.saveBtn')}
              </button>
            </form>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex flex-col">
      ...
      <div className="w-full flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className="text-sm text-gray-600">
              {t('settings.create3DElementDesc')}
            </p>
            <p className="text-xs text-gray-400 mt-1">
              {t('settings.elements', { count: elements.length, max: MAX_ELEMENTS })}
              {elements.length >= MAX_ELEMENTS && <span className="text-red-500 ml-1">{t('common.limitReached')}</span>}
            </p>
          </div>
          ...
          >
            ...
            <p className="text-sm text-gray-600">
              {t('settings.uploadModelDesc')}
            </p>
            <p className="text-xs text-gray-400 mt-1">
              {t('settings.models', { count: userModelsCount, max: MAX_MODELS })}
              {userModelsCount >= MAX_MODELS && <span className="text-red-500 ml-1">{t('common.limitReached')}</span>}
            </p>
          </div>
        ...
        {elements.length > 0 && (
          <div className="mt-12">
            <h3 className="text-xl font-bold text-gray-800 mb-4">{t('settings.my3DElements', { count: elements.length })}</h3>
        ...
        {uploadedModels.length > 0 && (
          <div className="mt-12">
            <h3 className="text-xl font-bold text-gray-800 mb-4">{t('settings.uploaded3DModels', { count: uploadedModels.length })}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {uploadedModels.map((model) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {t('settings.scaleLabel', { scale: model.modelScale, filename: model.modelFileName })}
                    </p>
                  </div>
                  <div className="flex flex-col gap-1">
                    <button
